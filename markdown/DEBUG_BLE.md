...
| 参数 | 值 | 说明 |
|---|---|---|
| I2S 采样率 | 8000 Hz | I2S SAMPLE_RATE（INMP441 SCK 下限约束） |
| BLE 输出采样率 | 2000 Hz | 硬件侧 4:1 抽取后输出 |
...
|---|---|---|
| 直流去除窗口 | 1000 样本 | 滑动平均 |
| 抗混叠 LPF 系数 α | 0.47 | 2阶 IIR 级联，截止 ~800Hz @ 8000Hz |
| 抽取倍数 | 4:1 | 8000Hz → 2000Hz，LPF 后抽取 |
| 数字增益 | 30× | 抽取后限幅前应用 |
| 限幅范围 | ±32767 | 16-bit 保护 |

### BLE 传输
| 参数 | 值 | 说明 |
|---|---|---|
| 单包大小 | 128 字节 | OUT_BLOCK_BYTES |
| 每包样本数 | 64 samples | 128 / 2字节，@ 2000Hz |
| 理论包速率 | 31.25 包/秒 | 2000 / 64 |
| 理论带宽 | 3.906 KB/s | 31.25 × 128 |
| **实测带宽** | **3.62 KB/s** | ble_debug 模式3 实测（939包，117.38KB，~32s） |
| BLE 传输开销 | ~7% | 理论 3.906 KB/s vs 实测 3.62 KB/s |
...
| 参数 | 值 | 说明 |
|---|---|---|
| 每段所需字节数 | 8000 字节 | 2000Hz × 2字节 × 2秒 |
| 实际采集耗时 | ~2.17 秒 | 8000 / 3707（3.62×1024） |
| 模型输入样本数 | 4000 samples | 2000Hz × 2s，ESP32 已完成降采样，Pi 端无需处理 |
